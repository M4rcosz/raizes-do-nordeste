...
export class DeactivatePromotionUseCase {
  private readonly logger = new Logger(DeactivatePromotionUseCase.name);
  ...
  async execute(promotionId: string, actor: PromotionActor, actorId: string): Promise<Promotion> {
    ...
    // Idempotent: deactivating an already-inactive promotion is a no-op write.
    const updated = await this.promotions.update(promotionId, { isActive: false });
    ...
    await this.tryAudit({
      ...
      entity: 'Promotion',
      entityId: updated.id,
      metadata: { businessUnitId: updated.businessUnitId, isActive: false },
    });
  ...
  private async tryAudit(input: AuditLogInput): Promise<void> {
    try {
      ...
      this.logger.warn({
        message: 'Audit logger threw during promotion deactivation; swallowed',
        action: input.action,
